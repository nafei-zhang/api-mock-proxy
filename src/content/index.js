...
if (document.documentElement.nodeName === 'HTML') {
  bootstrap().then((isActive) => {
    if (isActive) {
      console.log(
        "%capi-mock-proxy will try to inject itself into page (and all iframes in it, don't worry if you see this message multiple times therefore). Extension attempts to inject itself into the page in the fastest ways possible to start catching the requests early. If the browser blocks such injection it falls-back on slower injection methods. If you see Content Security Policy errors, it means that the browser blocked some of the fastest injection attempts: might be api-mock-proxy will not be able to catch the earliest requests sent by the page.",
        'color: #1f41df;',
      );
      ...
      scriptWithURL.onload = function () {
        this.remove();
        console.log('%capi-mock-proxy finished injecting.', 'color: #1f41df;');
      };
