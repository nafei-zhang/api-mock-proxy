...
const Router = ({
  ...
  const mainContent = analysedRequest ? (
    <div className="wmax navigation-node router-main pt2">
      <Button
        tretiary
      ...
      <Analyser request={analysedRequest} overrides={onlyCurrentDomainNodes} />
    </div>
  ) : Node ? (
    <Node
      {...selectedNode}
      toggle={() => toggleNode(selectedNode?.id)}
      className="wmax navigation-node router-main py2"
    />
  ) : nodes?.length ? (
    <div className="router-main router-main_empty">
      <Image404 />
    </div>
  ) : (
    ''
