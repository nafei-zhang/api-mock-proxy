...
const RequestsList = ({ className, onSelect, onAnalyse, ...otherProps }) => {
  ...
  return (
    <div className={cn('wmax', className)}>
      <h3 className="requests-list__title">Requests</h3>
      <p className="requests-list__subtitle">Select a request to override</p>
      <div className="requests-list__toolbar">
        <div className="ffr requests-list__search-row">
          <Input
            label="Search"
        ...
        <div className="filter-switch requests-list__switch-row">
          <Switch
            onChange={() => {
              const copy = [...searchTokens];
              const old = copy.pop();
      ...
      <div className="requests-list__requests">
        {aggregated &&
