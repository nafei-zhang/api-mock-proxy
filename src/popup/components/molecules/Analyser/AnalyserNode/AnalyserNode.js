...
const AnalyserSection = ({ name, matches }) => (
  <Section
    key={name}
    className="analyser-node__section"
    header={
      <Pill
        text={name}
        className={cn({
  ...
  >
    {matches?.map(({ request, override, isMatch, variable }) => (
      <div className={cn('analyser-node__row analyser-node__row_data label_medium')}>
...
const AnalyserNode = ({ name, url, method, headers, body }) => {
  ...
  return (
    ...
    >
      <div className="analyser-node__content">
