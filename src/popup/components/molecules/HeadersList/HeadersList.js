...
const HeadersList = ({ headers, onChange, className }) => {
  ...
  return (
    <div className={cn(className, 'headers-list')}>
      <Input
        label="Search"
        value={searchValue}
      ...
      <ul>
        {!filteredHeaders.length && headerValuesWithId.length && (
          <h4 className="headers-list__empty g2-color">No headers found</h4>
        )}
        {filteredHeaders?.map(({ id, name, value }, index) => (
          <li key={id} className="ffr headers-list__row">
            <Input
              className="w100 header__input_name"
              label={index === 0 ? 'Name' : ''}
              value={name}
              onChange={(newValue) =>
